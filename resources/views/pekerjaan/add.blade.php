@extends('layout')
@section('content')
  <div class="title text-center fs-3 fw-semibold mb-3">Tambah Pekerjaan</div>
  <form method="POST" action="{{ route('pekerjaan.store') }}">
    @csrf
      <div class="mb-3">
        <label for="w_id" class="form-label">ID Pekerjaan</label>
        <input type="text" id="w_id" class="form-control" name="w_id">
      </div>
      <div class="mb-3">
        <label for="w_nama" class="form-label">Nama Pekerjaan</label>
        <input type="text" id="w_nama" class="form-control" name="w_nama">
      </div>
      <div class="mb-3">
        ...
        <select id="d_id" class="form-select" name="d_id">
          @foreach ($dataDepartemen as $departemen)
            <option value={{ $departemen->d_id }}>{{ $departemen->d_nama }}</option>
          @endforeach
        </select>
      </div>
